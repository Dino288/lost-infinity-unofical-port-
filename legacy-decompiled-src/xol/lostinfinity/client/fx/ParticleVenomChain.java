...
public class ParticleVenomChain
extends LostParticle {
    private static final Random RANDOM = new Random();

    protected ParticleVenomChain(Level worldIn, double xCoordIn, double yCoordIn, double zCoordIn, double xSpeedIn, double ySpeed, double zSpeedIn) {
        ...
        if (xSpeedIn == 0.0 && zSpeedIn == 0.0) {
            this.field_187129_i *= 0.01;
            this.field_187131_k *= 0.01;
        }
        this.scale = 0.8f;
        this.field_70547_e = (int)(5.0 / (Math.random() * 0.8 + 0.2));
        this.func_187117_a(ParticleInit.VENOM_CHAIN_SPRITE);
    }

    public ParticleVenomChain setParticleGravity(double motionY) {
    ...
    public void func_189213_a() {
        ...
        if (this.field_70547_e - this.field_70546_d < 8) {
            if ((double)this.scale > 0.5) {
                ...
                this.scale = (float)((double)this.scale - 0.1);
            }
        } else {
            this.prevScale = this.scale;
        }
    ...
    implements IParticleFactory {
        ...
        public Particle func_178902_a(int particleID, Level world, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed, int ... p_178902_15_) {
            return new ParticleVenomChain(world, x, y, z, xSpeed, ySpeed, zSpeed);
        }
    }
}
