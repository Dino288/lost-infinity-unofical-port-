...
import xol.lostinfinity.util.compatibility.jei.trades.blackmarket.BlackMarketTradeJEI;

public class BlackMarketTradeWrapper
implements IRecipeWrapper {
    private final List<ItemStack> inputs;
    private final ItemStack output;

    public BlackMarketTradeWrapper(List<ItemStack> inputs, ItemStack output) {
        this.inputs = inputs;
        this.output = output;
    }
    ...
    public static List<BlackMarketTradeWrapper> getRecipes(IJeiHelpers helpers) {
        IStackHelper stackHelper = helpers.getStackHelper();
        BlackMarketTradeJEI instance = BlackMarketTradeJEI.getInstance();
        Table<ItemStack, ItemStack, ItemStack> recipes = instance.getBlackMarketTradeList();
        ...
        for (Map.Entry entry : recipes.columnMap().entrySet()) {
            for (Map.Entry ent : ((Map)entry.getValue()).entrySet()) {
                ...
                BlackMarketTradeWrapper recipe = new BlackMarketTradeWrapper(inputs, output);
