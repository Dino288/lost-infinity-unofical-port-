...
import xol.lostinfinity.util.compatibility.jei.trades.chemist.ChemistTradeJEI;

public class ChemistTradeWrapper
implements IRecipeWrapper {
    private final List<ItemStack> inputs;
    private final ItemStack output;

    public ChemistTradeWrapper(List<ItemStack> inputs, ItemStack output) {
        this.inputs = inputs;
        this.output = output;
    }
    ...
    public static List<ChemistTradeWrapper> getRecipes(IJeiHelpers helpers) {
        IStackHelper stackHelper = helpers.getStackHelper();
        ChemistTradeJEI instance = ChemistTradeJEI.getInstance();
        Table<ItemStack, ItemStack, ItemStack> recipes = instance.getChemistTradeList();
        ...
        for (Map.Entry entry : recipes.columnMap().entrySet()) {
            for (Map.Entry ent : ((Map)entry.getValue()).entrySet()) {
                ...
                ChemistTradeWrapper recipe = new ChemistTradeWrapper(inputs, output);
