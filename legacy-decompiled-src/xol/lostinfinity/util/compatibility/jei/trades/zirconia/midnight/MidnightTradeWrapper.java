...
import xol.lostinfinity.util.compatibility.jei.trades.zirconia.midnight.MidnightTradeJEI;

public class MidnightTradeWrapper
implements IRecipeWrapper {
    private final List<ItemStack> inputs;
    private final ItemStack output;

    public MidnightTradeWrapper(List<ItemStack> inputs, ItemStack output) {
        this.inputs = inputs;
        this.output = output;
    }
    ...
    public static List<MidnightTradeWrapper> getRecipes(IJeiHelpers helpers) {
        IStackHelper stackHelper = helpers.getStackHelper();
        MidnightTradeJEI instance = MidnightTradeJEI.getInstance();
        ...
        for (Map.Entry entry : recipes.columnMap().entrySet()) {
            for (Map.Entry ent : ((Map)entry.getValue()).entrySet()) {
                ...
                MidnightTradeWrapper recipe = new MidnightTradeWrapper(inputs, output);
