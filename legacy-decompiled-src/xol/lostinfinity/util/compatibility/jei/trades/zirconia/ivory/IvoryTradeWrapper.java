...
import xol.lostinfinity.util.compatibility.jei.trades.zirconia.ivory.IvoryTradeJEI;

public class IvoryTradeWrapper
implements IRecipeWrapper {
    private final List<ItemStack> inputs;
    private final ItemStack output;

    public IvoryTradeWrapper(List<ItemStack> inputs, ItemStack output) {
        this.inputs = inputs;
        this.output = output;
    }
    ...
    public static List<IvoryTradeWrapper> getRecipes(IJeiHelpers helpers) {
        IStackHelper stackHelper = helpers.getStackHelper();
        IvoryTradeJEI instance = IvoryTradeJEI.getInstance();
        ...
        for (Map.Entry entry : recipes.columnMap().entrySet()) {
            for (Map.Entry ent : ((Map)entry.getValue()).entrySet()) {
                ...
                IvoryTradeWrapper recipe = new IvoryTradeWrapper(inputs, output);
