...
import xol.lostinfinity.util.compatibility.jei.trades.zirconia.rosewood.RosewoodTradeJEI;

public class RosewoodTradeWrapper
implements IRecipeWrapper {
    private final List<ItemStack> inputs;
    private final ItemStack output;

    public RosewoodTradeWrapper(List<ItemStack> inputs, ItemStack output) {
        this.inputs = inputs;
        this.output = output;
    }
    ...
    public static List<RosewoodTradeWrapper> getRecipes(IJeiHelpers helpers) {
        IStackHelper stackHelper = helpers.getStackHelper();
        RosewoodTradeJEI instance = RosewoodTradeJEI.getInstance();
        ...
        for (Map.Entry entry : recipes.columnMap().entrySet()) {
            for (Map.Entry ent : ((Map)entry.getValue()).entrySet()) {
                ...
                RosewoodTradeWrapper recipe = new RosewoodTradeWrapper(inputs, output);
