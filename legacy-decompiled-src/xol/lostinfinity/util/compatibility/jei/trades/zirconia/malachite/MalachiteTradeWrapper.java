...
import xol.lostinfinity.util.compatibility.jei.trades.zirconia.malachite.MalachiteTradeJEI;

public class MalachiteTradeWrapper
implements IRecipeWrapper {
    private final List<ItemStack> inputs;
    private final ItemStack output;

    public MalachiteTradeWrapper(List<ItemStack> inputs, ItemStack output) {
        this.inputs = inputs;
        this.output = output;
    }
    ...
    public static List<MalachiteTradeWrapper> getRecipes(IJeiHelpers helpers) {
        IStackHelper stackHelper = helpers.getStackHelper();
        MalachiteTradeJEI instance = MalachiteTradeJEI.getInstance();
        ...
        for (Map.Entry entry : recipes.columnMap().entrySet()) {
            for (Map.Entry ent : ((Map)entry.getValue()).entrySet()) {
                ...
                MalachiteTradeWrapper recipe = new MalachiteTradeWrapper(inputs, output);
