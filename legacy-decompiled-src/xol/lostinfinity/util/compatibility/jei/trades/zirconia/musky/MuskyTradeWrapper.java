...
import xol.lostinfinity.util.compatibility.jei.trades.zirconia.musky.MuskyTradeJEI;

public class MuskyTradeWrapper
implements IRecipeWrapper {
    private final List<ItemStack> inputs;
    private final ItemStack output;

    public MuskyTradeWrapper(List<ItemStack> inputs, ItemStack output) {
        this.inputs = inputs;
        this.output = output;
    }
    ...
    public static List<MuskyTradeWrapper> getRecipes(IJeiHelpers helpers) {
        IStackHelper stackHelper = helpers.getStackHelper();
        MuskyTradeJEI instance = MuskyTradeJEI.getInstance();
        ...
        for (Map.Entry entry : recipes.columnMap().entrySet()) {
            for (Map.Entry ent : ((Map)entry.getValue()).entrySet()) {
                ...
                MuskyTradeWrapper recipe = new MuskyTradeWrapper(inputs, output);
