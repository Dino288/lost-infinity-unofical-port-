...
import xol.lostinfinity.util.compatibility.jei.trades.zirconia.crimson.CrimsonTradeJEI;

public class CrimsonTradeWrapper
implements IRecipeWrapper {
    private final List<ItemStack> inputs;
    private final ItemStack output;

    public CrimsonTradeWrapper(List<ItemStack> inputs, ItemStack output) {
        this.inputs = inputs;
        this.output = output;
    }
    ...
    public static List<CrimsonTradeWrapper> getRecipes(IJeiHelpers helpers) {
        IStackHelper stackHelper = helpers.getStackHelper();
        CrimsonTradeJEI instance = CrimsonTradeJEI.getInstance();
        ...
        for (Map.Entry entry : recipes.columnMap().entrySet()) {
            for (Map.Entry ent : ((Map)entry.getValue()).entrySet()) {
                ...
                CrimsonTradeWrapper recipe = new CrimsonTradeWrapper(inputs, output);
