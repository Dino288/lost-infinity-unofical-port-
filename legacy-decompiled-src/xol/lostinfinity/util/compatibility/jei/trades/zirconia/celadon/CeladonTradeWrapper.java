...
import xol.lostinfinity.util.compatibility.jei.trades.zirconia.celadon.CeladonTradeJEI;

public class CeladonTradeWrapper
implements IRecipeWrapper {
    private final List<ItemStack> inputs;
    private final ItemStack output;

    public CeladonTradeWrapper(List<ItemStack> inputs, ItemStack output) {
        this.inputs = inputs;
        this.output = output;
    }
    ...
    public static List<CeladonTradeWrapper> getRecipes(IJeiHelpers helpers) {
        IStackHelper stackHelper = helpers.getStackHelper();
        CeladonTradeJEI instance = CeladonTradeJEI.getInstance();
        ...
        for (Map.Entry entry : recipes.columnMap().entrySet()) {
            for (Map.Entry ent : ((Map)entry.getValue()).entrySet()) {
                ...
                CeladonTradeWrapper recipe = new CeladonTradeWrapper(inputs, output);
