...
import xol.lostinfinity.util.compatibility.jei.nicroniuminfuser.NicroniumInfuserRecipeJEI;

public class NicroniumInfuserRecipeWrapper
implements IRecipeWrapper {
    private final List<ItemStack> inputs;
    private final ItemStack output;

    public NicroniumInfuserRecipeWrapper(List<ItemStack> inputs, ItemStack output) {
        this.inputs = inputs;
        this.output = output;
    }
    ...
    public static List<NicroniumInfuserRecipeWrapper> getRecipes(IJeiHelpers helpers) {
        IStackHelper stackHelper = helpers.getStackHelper();
        NicroniumInfuserRecipeJEI instance = NicroniumInfuserRecipeJEI.getInstance();
        Table<ItemStack, ItemStack, ItemStack> recipes = instance.getNicroniumInfuserList();
        ...
        for (Map.Entry entry : recipes.columnMap().entrySet()) {
            for (Map.Entry ent : ((Map)entry.getValue()).entrySet()) {
                ...
                NicroniumInfuserRecipeWrapper recipe = new NicroniumInfuserRecipeWrapper(inputs, output);
