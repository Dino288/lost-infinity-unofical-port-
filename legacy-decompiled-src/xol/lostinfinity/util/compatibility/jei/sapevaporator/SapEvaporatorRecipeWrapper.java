...
import xol.lostinfinity.util.compatibility.jei.sapevaporator.SapEvaporatorRecipeJEI;

public class SapEvaporatorRecipeWrapper
implements IRecipeWrapper {
    private final List<ItemStack> inputs;
    private final ItemStack output;

    public SapEvaporatorRecipeWrapper(List<ItemStack> inputs, ItemStack output) {
        this.inputs = inputs;
        this.output = output;
    }
    ...
    public static List<SapEvaporatorRecipeWrapper> getRecipes(IJeiHelpers helpers) {
        IStackHelper stackHelper = helpers.getStackHelper();
        SapEvaporatorRecipeJEI instance = SapEvaporatorRecipeJEI.getInstance();
        Table<ItemStack, ItemStack, ItemStack> recipes = instance.getSapEvaporatorList();
        ...
        for (Map.Entry entry : recipes.columnMap().entrySet()) {
            for (Map.Entry ent : ((Map)entry.getValue()).entrySet()) {
                ...
                SapEvaporatorRecipeWrapper recipe = new SapEvaporatorRecipeWrapper(inputs, output);
