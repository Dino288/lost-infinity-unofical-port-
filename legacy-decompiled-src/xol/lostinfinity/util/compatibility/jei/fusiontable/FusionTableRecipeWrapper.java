...
import xol.lostinfinity.util.compatibility.jei.fusiontable.FusionTableRecipeJEI;

public class FusionTableRecipeWrapper
implements IRecipeWrapper {
    private final List<ItemStack> inputs;
    private final ItemStack output;

    public FusionTableRecipeWrapper(List<ItemStack> inputs, ItemStack output) {
        this.inputs = inputs;
        this.output = output;
    }
    ...
    public static List<FusionTableRecipeWrapper> getRecipes(IJeiHelpers helpers) {
        IStackHelper stackHelper = helpers.getStackHelper();
        FusionTableRecipeJEI instance = FusionTableRecipeJEI.getInstance();
        Table<ItemStack, ItemStack, ItemStack> recipes = instance.getFusionTableList();
        ...
        for (Map.Entry entry : recipes.columnMap().entrySet()) {
            for (Map.Entry ent : ((Map)entry.getValue()).entrySet()) {
                ...
                FusionTableRecipeWrapper recipe = new FusionTableRecipeWrapper(inputs, output);
