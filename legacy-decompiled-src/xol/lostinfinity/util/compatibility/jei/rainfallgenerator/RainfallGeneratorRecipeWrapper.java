...
import xol.lostinfinity.util.compatibility.jei.rainfallgenerator.RainfallGeneratorRecipeJEI;

public class RainfallGeneratorRecipeWrapper
implements IRecipeWrapper {
    private final List<ItemStack> inputs;
    private final ItemStack output;

    public RainfallGeneratorRecipeWrapper(List<ItemStack> inputs, ItemStack output) {
        this.inputs = inputs;
        this.output = output;
    }
    ...
    public static List<RainfallGeneratorRecipeWrapper> getRecipes(IJeiHelpers helpers) {
        IStackHelper stackHelper = helpers.getStackHelper();
        RainfallGeneratorRecipeJEI instance = RainfallGeneratorRecipeJEI.getInstance();
        Table<ItemStack, ItemStack, ItemStack> recipes = instance.getRainfallGeneratorList();
        ...
        for (Map.Entry entry : recipes.columnMap().entrySet()) {
            for (Map.Entry ent : ((Map)entry.getValue()).entrySet()) {
                ...
                RainfallGeneratorRecipeWrapper recipe = new RainfallGeneratorRecipeWrapper(inputs, output);
