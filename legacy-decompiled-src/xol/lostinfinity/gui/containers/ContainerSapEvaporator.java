...
import xol.lostinfinity.gui.slots.SlotSapEvaporatorFuel;
import xol.lostinfinity.gui.slots.SlotSapEvaporatorOutput;
import xol.lostinfinity.recipes.SapEvaporatorRecipes;

public class ContainerSapEvaporator
extends Container {
    private final BlockEntitySapEvaporator tileentity;
    private int cookTime;
    private int totalCookTime;
    private int burnTime;
    private int currentBurnTime;

    public ContainerSapEvaporator(Inventory player, BlockEntitySapEvaporator tileentity) {
        this.tileentity = tileentity;
        this.func_75146_a(new Slot((IInventory)tileentity, 0, 17, 21));
        this.func_75146_a(new SlotSapEvaporatorFuel(tileentity, 1, 17, 57));
        this.func_75146_a(new SlotSapEvaporatorOutput(player.field_70458_d, tileentity, 2, 98, 39));
        for (int y = 0; y < 3; ++y) {
            for (int x = 0; x < 9; ++x) {
    ...
    public ItemStack func_82846_b(Player playerIn, int index) {
        ...
        if (slot != null && slot.func_75216_d()) {
            ...
            } else if (index != 1 && index != 0 ? (SapEvaporatorRecipes.getResult(itemstack1) != null && !SapEvaporatorRecipes.getResult(itemstack1).func_190926_b() ? !this.func_75135_a(itemstack1, 0, 1, false) : (BlockEntitySapEvaporator.isItemFuel(itemstack1) ? !this.func_75135_a(itemstack1, 1, 2, false) : (index >= 3 && index < 30 ? !this.func_75135_a(itemstack1, 30, 39, false) : index >= 30 && index < 39 && !this.func_75135_a(itemstack1, 3, 30, false)))) : !this.func_75135_a(itemstack1, 3, 39, false)) {
                return ItemStack.field_190927_a;
            }
