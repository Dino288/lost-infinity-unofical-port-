...
public class RenderGalaxyGladiator
extends RenderLiving<EntityGalaxyGladiator> {
    ...
    public RenderGalaxyGladiator(RenderManager manager) {
        super(manager, (ModelBase)new ModelGalaxyGladiator(), 0.5f);
    }

    protected void preRenderCallback(EntityGalaxyGladiator entitylivingbaseIn, float partialTickTime) {
        GlStateManager.func_179152_a((float)this.scale, (float)this.scale, (float)this.scale);
    }

    protected ResourceLocation getEntityTexture(EntityGalaxyGladiator entity) {
    ...
    protected void applyRotations(EntityGalaxyGladiator entityLiving, float p1, float rotationYaw, float partialTicks) {
