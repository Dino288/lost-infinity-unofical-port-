...
public class RenderDeviantCow
extends RenderLiving<EntityDeviantCow> {
    ...
    public RenderDeviantCow(RenderManager manager) {
        super(manager, (ModelBase)new ModelDeviantCow(), 0.5f);
    }

    protected void preRenderCallback(EntityDeviantCow entitylivingbaseIn, float partialTickTime) {
        GlStateManager.func_179152_a((float)this.scale, (float)this.scale, (float)this.scale);
    }

    protected ResourceLocation getEntityTexture(EntityDeviantCow entity) {
    ...
    protected void applyRotations(EntityDeviantCow entityLiving, float p1, float rotationYaw, float partialTicks) {
