...
public class RenderDeviantGolem
extends RenderLiving<EntityDeviantGolem> {
    ...
    public RenderDeviantGolem(RenderManager manager) {
        super(manager, (ModelBase)new ModelDeviantGolem(), 0.5f);
    }

    protected void preRenderCallback(EntityDeviantGolem entitylivingbaseIn, float partialTickTime) {
        GlStateManager.func_179152_a((float)this.scale, (float)this.scale, (float)this.scale);
    }

    protected ResourceLocation getEntityTexture(EntityDeviantGolem entity) {
    ...
    protected void applyRotations(EntityDeviantGolem entityLiving, float p1, float rotationYaw, float partialTicks) {
