...
public class RenderTrialObserver
extends RenderLiving<EntityTrialObserver> {
    ...
    public RenderTrialObserver(RenderManager manager) {
        super(manager, (ModelBase)new ModelTrialObserver(), 0.5f);
    }

    protected void preRenderCallback(EntityTrialObserver entitylivingbaseIn, float partialTickTime) {
        GlStateManager.func_179152_a((float)this.scale, (float)this.scale, (float)this.scale);
    }

    protected ResourceLocation getEntityTexture(EntityTrialObserver entity) {
        return TEXTURES;
    }

    protected void applyRotations(EntityTrialObserver entityLiving, float p1, float rotationYaw, float partialTicks) {
