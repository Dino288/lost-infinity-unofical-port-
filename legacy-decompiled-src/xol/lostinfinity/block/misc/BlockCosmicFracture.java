...
public class BlockCosmicFracture
extends Block
...
ITetherable {
    ...
    public BlockCosmicFracture(String name, float hardness, Material material, CreativeModeTab tab) {
        super(material);
    ...
    public void func_180634_a(Level worldIn, BlockPos pos, BlockState state, Entity entityIn) {
        if (!worldIn.field_72995_K && entityIn instanceof LivingEntity && !(entityIn instanceof EntityImmaterial) && entityIn.field_70173_aa % 4 == 2) {
            if (entityIn instanceof Player && ((Player)entityIn).func_70644_a(PotionInit.FRACTURE)) {
                return;
            }
            IMaxAttack.dealTrueDamage(null, (LivingEntity)entityIn, ((LivingEntity)entityIn).func_110138_aP() / 3.0f);
        }
    }
